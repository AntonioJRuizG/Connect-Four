...
function setTopTile() {
    if (gameOver !== true) {
        document.getElementById(this.id.split('-')[1]).style.backgroundColor = currentPlayer
    }
}
...
function PCMoveTile() {
    ...
    setTimeout(() => { // Little delay on PC turn to improve user experience
        ...
        for (let i = rows - 1; i >= 0; i--) {
            if (board[i][PCColumSelection] === ' ') {
                document.getElementById(i + '-' + PCColumSelection).classList.add(currentPlayer)
